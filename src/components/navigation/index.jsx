...
const Navigation = () => {
  ...
  return (
    // FIXED: Added pointer-events-none and higher z-index
    <div className="w-full fixed h-screen flex items-center justify-center pointer-events-none z-30">
      <ResponsiveComponent>
        {({ size }) => {
          return size && size >= 480 ? (
            // FIXED: Re-enable pointer events only for the actual navigation
            <motion.div
              variants={container}
              initial="hidden"
              animate="show"
            ...
            <>
              {/* FIXED: Re-enable pointer events for mobile navigation */}
              <motion.div
                variants={container}
                initial="hidden"
                animate="show"
